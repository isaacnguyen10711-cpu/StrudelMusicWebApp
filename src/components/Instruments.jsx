            ...
            {instrumentalList.length > 0 && (
                instrumentalList.map((instrument, index) => (
                    <div key={index} className="form-check form-switch mb-2">
                        <input className="form-check-input mt-4" style={{ marginLeft: "120px" }} type="checkbox" id={`instrument_${index}` } checked={instrumentIsPlaying[index]} onChange={() => toggleInstrument(index)} />
